# swin_gru_with_source.py
# Swin-T (frame encoder) + Bi-GRU over time + video-source conditioning (one-hot 0..6)
...
SAVE_DIR = r"D:\UK\00. 2024 QMUL\00. Course\Project\00. ViolenceDetectionProject\Results\video_source\Swin_BiGRU"
...
#        MODEL          #
# ===================== #
class SwinBiGRUWithSource(nn.Module):
    """
    Swin-T frame encoder (head removed) -> per-frame 768-d features
    Bi-GRU over time -> mean pool over sequence
    Concatenate source one-hot -> linear classifier
    Freeze Swin stages 0-2, keep stage 3 trainable
    """
    def __init__(self, num_sources: int = 7, gru_hidden: int = 256, gru_layers: int = 1, dropout: float = 0.3):
        super().__init__()
        self.backbone = swin_t(weights=Swin_T_Weights.DEFAULT)
        self.embed_dim = self.backbone.head.in_features  # 768
        self.backbone.head = nn.Identity()

        # Freeze early Swin stages
        for s in [0, 1, 2]:
            for p in self.backbone.features[s].parameters():
                p.requires_grad = False

        # Bi-GRU over temporal features
        self.gru = nn.GRU(
        ...
        feat_out = gru_hidden * 2  # bidirectional
        self.fc = nn.Linear(feat_out + num_sources, 1)
    ...
    def forward(self, x: torch.Tensor, src_oh: torch.Tensor) -> torch.Tensor:
        ...
        seq_out, _ = self.gru(feats)                     # [B, T, 2*hidden]
        vid_feat = seq_out.mean(dim=1)                   # [B, 2*hidden]
        fused = torch.cat([vid_feat, src_oh], dim=1)     # [B, 2*hidden + 7]
...
# ===================== #
def main():
    print("[INFO] Training Swin + Bi-GRU with video-source conditioning...")
    ...
    model = SwinBiGRUWithSource(num_sources=7, gru_hidden=256, gru_layers=1, dropout=0.3).to(DEVICE)
    ...
    pd.DataFrame({
        ...
        "True": y_true,
        "Pred": y_pred
    }).to_csv(os.path.join(SAVE_DIR, "swin_bigru_with_source_test_predictions.csv"), index=False)

    pd.DataFrame(report).to_csv(os.path.join(SAVE_DIR, "swin_bigru_with_source_test_metrics.csv"))

    pd.DataFrame(cm, index=["True_Non-violent", "True_Violent"],
                 columns=["Pred_Non-violent", "Pred_Violent"]).to_csv(
        os.path.join(SAVE_DIR, "swin_bigru_with_source_confusion_matrix.csv")
    )

    with open(os.path.join(SAVE_DIR, "swin_bigru_with_source_test_bce_loss.txt"), "w") as f:
