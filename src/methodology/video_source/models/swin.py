# swin_only_with_source.py
# Swin-T (frame encoder only) + temporal mean pool + video-source conditioning (one-hot 0..6)
...
SAVE_DIR = r"D:\UK\00. 2024 QMUL\00. Course\Project\00. ViolenceDetectionProject\Results\video_source\Swin_Only"
...
#        MODEL          #
# ===================== #
class SwinMeanPoolWithSource(nn.Module):
    """
    ...
    """
    def __init__(self, num_sources: int = 7, dropout: float = 0.3):
        super().__init__()
        ...
        self.fc = nn.Linear(self.embed_dim + num_sources, 1)
    ...
    def forward(self, x: torch.Tensor, src_oh: torch.Tensor) -> torch.Tensor:
        ...
        vid_feat = feats.mean(dim=1)                     # [B, 768]
        fused = torch.cat([vid_feat, src_oh], dim=1)     # [B, 768 + 7]
...
# ===================== #
def main():
    print("[INFO] Training Swin-only with video-source conditioning...")
    ...
    model = SwinMeanPoolWithSource(num_sources=7, dropout=0.3).to(DEVICE)
    ...
    pd.DataFrame({
        ...
        "True": y_true,
        "Pred": y_pred
    }).to_csv(os.path.join(SAVE_DIR, "swin_only_with_source_test_predictions.csv"), index=False)

    # Save metrics
    pd.DataFrame(report).to_csv(os.path.join(SAVE_DIR, "swin_only_with_source_test_metrics.csv"))

    # Save confusion matrix
    pd.DataFrame(cm, index=["True_Non-violent", "True_Violent"],
                 columns=["Pred_Non-violent", "Pred_Violent"]).to_csv(
        os.path.join(SAVE_DIR, "swin_only_with_source_confusion_matrix.csv")
    )

    # Save brief text summary
    with open(os.path.join(SAVE_DIR, "swin_only_with_source_test_bce_loss.txt"), "w") as f:
